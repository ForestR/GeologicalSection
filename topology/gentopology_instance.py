...
def gen_topology(hi1, s1, ds1, nl1, nd1, hi2, s2, ds2, nl2, nd2, 
    ...
    for i in range(len(cd1)-1):
        ...
        if (n1 == 1) and (n2 == 1):
            curve_list.append([num_curve, nl1[i], nl2[i], 
                          s1[cd1[i]], s1[cd1[i+1]]])
            num_curve += 1
            
        elif (n1 > 1) and (n2 == 1):
            (num_node, new_node_list, num_curve, curve_list) = new_node_hole1(
                hi1, s1, nl1, nd1, cd1, i, n1, hi2,
                num_node, new_node_list, num_curve, curve_list)
            
            curve_list.append([num_curve, num_node, nl2[i],
            ...
            curve_list.append([num_curve, nl1[i], num_node,
                               s1[cd1[i]], s1[cd1[i]+n1]])
            num_curve += 1  
...
if __name__ == "__main__":
    ...
    hi1 = 1; hi2 = 2;
    nl1 = []
    for x in range(len(s1)-1): nl1.append(10+x)
    nd1 = []
    for x in range(len(s1)-1): nd1.append(0.0+1.5*x)
    
    nl2 = []
    for x in range(len(s2)-1): nl2.append(20+x)
    nd2 = []
